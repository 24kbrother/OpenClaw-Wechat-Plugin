...
// 代理配置
let wecomProxyConfig = null;

function getWecomProxyConfig(api) {
  if (wecomProxyConfig !== null) {
    return wecomProxyConfig;
  }
  ...
  // 从环境变量读取代理配置
  const proxyUrl = envVars.WECOM_API_PROXY?.trim() || process.env.WECOM_API_PROXY || null;
...
// 通用微信API调用函数（支持代理）
async function fetchWecomApi(api, url, options = {}) {
  const proxyConfig = getWecomProxyConfig(api);
  
  if (proxyConfig) {
    // 将URL转换为代理路径
    // 例如: https://qyapi.weixin.qq.com/cgi-bin/gettoken?xxx → /cgi-bin/gettoken?xxx
    let targetUrl;
    try {
...
async function getWecomAccessToken({ api, corpId, corpSecret }) {
  ...
  cache.refreshPromise = (async () => {
    try {
      ...
      const tokenRes = await fetchWecomApi(api, tokenUrl);
...
// 发送单条文本消息（内部函数，带限流）
async function sendWecomTextSingle({ api, corpId, corpSecret, agentId, toUser, text }) {
  return apiLimiter.execute(async () => {
    const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
    ...
    const sendRes = await fetchWecomApi(api, sendUrl, {
      method: "POST",
...
// 上传临时素材到企业微信
async function uploadWecomMedia({ api, corpId, corpSecret, type, buffer, filename }) {
  const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
  ...
  const res = await fetchWecomApi(api, uploadUrl, {
    method: "POST",
    headers: {
...
// 发送图片消息（带限流）
async function sendWecomImage({ api, corpId, corpSecret, agentId, toUser, mediaId }) {
  return apiLimiter.execute(async () => {
    const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
    const sendUrl = `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=${encodeURIComponent(accessToken)}`;
...
// 发送视频消息（带限流）
async function sendWecomVideo({ api, corpId, corpSecret, agentId, toUser, mediaId, title, description }) {
  return apiLimiter.execute(async () => {
    const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
    const sendUrl = `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=${encodeURIComponent(accessToken)}`;
    const body = {
...
// 发送文件消息（带限流）
async function sendWecomFile({ api, corpId, corpSecret, agentId, toUser, mediaId }) {
  return apiLimiter.execute(async () => {
    const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
    const sendUrl = `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=${encodeURIComponent(accessToken)}`;
    const body = {
...
const WecomChannelPlugin = {
  ...
  meta: {
    ...
    aliases: ["openclaw-wechat-plugin", "openclaw-wecom", "wecom", "wework", "qiwei", "wxwork"],
  },
  capabilities: {
...
// 下载企业微信媒体文件
async function downloadWecomMedia({ api, corpId, corpSecret, mediaId }) {
  const accessToken = await getWecomAccessToken({ api, corpId, corpSecret });
  ...
  const res = await fetchWecomApi(api, mediaUrl);
  if (!res.ok) {
...
支持发送图片，AI 会分析图片内容。`;
  ...
  await sendWecomText({ api, corpId, corpSecret, agentId, toUser: fromUser, text: helpText });
  return true;
}
...
✅ 多账户支持`;
  ...
  await sendWecomText({ api, corpId, corpSecret, agentId, toUser: fromUser, text: statusText });
  return true;
}
